...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function startVisualizer() {
        ...
        // Keep track of ticks for simulated visualizer
        let simulatedTick = 0;
        ...
        function draw() {
            ...
            // Base radius of visualizer circle (matches vinyl outer border approx 100px)
            const baseRadius = 104;

            // Fetch frequency data if Web Audio API is active
            let activeData = false;
            if (analyser && !isUsingSimulatedVisualizer) {
                analyser.getByteFrequencyData(dataArray);
                
                // Check if we get only 0s (due to CORS block on the stream)
                let sum = 0;
                for (let i = 0; i < bufferLength; i++) {
                    sum += dataArray[i];
                }
                if (sum > 0) {
                    isAnalyserSilent = false;
                    activeData = true;
                } else if (!isAnalyserSilent) {
                    // It was working but became silent (paused or loading)
                    activeData = true;
                }
            }

            // Fallback to simulation if AudioContext is blocked or silent (CORS)
            if (!activeData) {
                isUsingSimulatedVisualizer = true;
                simulatedTick += isPlaying ? 0.05 : 0.005;
                
                // Draw simulated circular pulse waves
                const pulseCount = 3;
                for (let i = 0; i < pulseCount; i++) {
                    const waveOffset = (simulatedTick + (i / pulseCount)) % 1;
                    const alpha = (1 - waveOffset) * (isPlaying ? 0.4 : 0.1);
                    const radius = baseRadius + (waveOffset * 60);

                    canvasCtx.beginPath();
                    canvasCtx.arc(centerX, centerY, radius, 0, 2 * Math.PI);
                    canvasCtx.strokeStyle = i % 2 === 0 ? `rgba(0, 255, 204, ${alpha})` : `rgba(255, 0, 127, ${alpha})`;
                    canvasCtx.lineWidth = 2;
                    canvasCtx.shadowBlur = 10;
                    canvasCtx.shadowColor = i % 2 === 0 ? "rgba(0, 255, 204, 0.5)" : "rgba(255, 0, 127, 0.5)";
                    canvasCtx.stroke();
                }
                
                // Draw a few outer floating particles for extra flavor
                const particles = 12;
                canvasCtx.shadowBlur = 0;
                for (let p = 0; p < particles; p++) {
                    const angle = (p / particles) * Math.PI * 2 + (simulatedTick * 0.1);
                    const speed = isPlaying ? Math.sin(simulatedTick + p) * 12 : 2;
                    const r = baseRadius + 15 + speed;
                    const x = centerX + Math.cos(angle) * r;
                    const y = centerY + Math.sin(angle) * r;
                    
                    canvasCtx.beginPath();
                    canvasCtx.arc(x, y, 2, 0, 2 * Math.PI);
                    canvasCtx.fillStyle = p % 2 === 0 ? "rgba(0, 255, 204, 0.6)" : "rgba(255, 0, 127, 0.6)";
                    canvasCtx.fill();
                }
                return;
            }

            // --- REAL AUDIO VISUALIZER (Radial Frequency Bars) ---
            isUsingSimulatedVisualizer = false;
            const barCount = 80;
            ...
            for (let i = 0; i < barCount; i++) {
                // Map bar index to frequency array (logarithmic focus on low/mid frequencies)
                const dataIndex = Math.floor(Math.pow(i / barCount, 1.5) * (bufferLength * 0.8));
                ...
                // Color gradient based on angle / index
